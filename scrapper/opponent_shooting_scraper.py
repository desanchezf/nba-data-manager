#!/usr/bin/env python3
"""
Scraper especializado para datos de Opponent Shooting de NBA
"""
...
class OpponentShootingScraper(BaseNBAScraper):
    """
    Scraper especializado para datos de Opponent Shooting
    """

    def get_table_selector(self):
        """
        Selector específico para la tabla de Opponent Shooting
        """
        return "table[data-module='Opponentshooting']"

    def extract_data(self, url):
        """
        Extrae datos específicos de Opponent Shooting
        ...
        try:
            ...
            if rows and headers:
                ...
                df = df.with_columns(pl.lit("opponent_shooting").alias("category"))
            ...
            logger.error(f"Error extrayendo datos de Opponent Shooting: {e}")
            return pl.DataFrame()

    def _clean_data(self, df):
        """
        Limpia y procesa los datos específicos de Opponent Shooting
        ...
        """
        try:
            # Convertir columnas numéricas específicas de Opponent Shooting
            numeric_columns = [
