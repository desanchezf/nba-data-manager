#!/usr/bin/env python3
"""
Scraper especializado para datos de Box Outs de NBA
"""
...
class BoxOutsScraper(BaseNBAScraper):
    """
    Scraper especializado para datos de Box Outs
    """

    def get_table_selector(self):
        """
        Selector específico para la tabla de Box Outs
        """
        return "table[data-module='Boxouts']"

    def extract_data(self, url):
        """
        Extrae datos específicos de Box Outs
        ...
        try:
            ...
            if rows and headers:
                ...
                df = df.with_columns(pl.lit("box_outs").alias("category"))
            ...
            logger.error(f"Error extrayendo datos de Box Outs: {e}")
            return pl.DataFrame()

    def _clean_data(self, df):
        """
        Limpia y procesa los datos específicos de Box Outs
        ...
        """
        try:
            # Convertir columnas numéricas específicas de Box Outs
            numeric_columns = [
